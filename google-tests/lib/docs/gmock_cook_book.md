...
int Process(const MyData& data_);
string DoSomething();

class MockFoo : public Foo {
 public:
  MOCK_METHOD(void, Abc, (const MyData& data_), (override));
...
class Buzzer {
 public:
  ...
  virtual bool ShareBuzz(std::unique_ptr<Buzz> buzz, int64_t timestamp_) = 0;
  ...
};
```
...
class MockBuzzer : public Buzzer {
 public:
  ...
  MOCK_METHOD(bool, ShareBuzz, (std::unique_ptr<Buzz> buzz, int64_t timestamp_),
              (override));
};
```
...
class MockBuzzer : public Buzzer {
 public:
  MOCK_METHOD(bool, DoShareBuzz, (Buzz* buzz, Time timestamp_));
  bool ShareBuzz(std::unique_ptr<Buzz> buzz, Time timestamp_) override {
    return DoShareBuzz(buzz.get(), timestamp_);
  }
};
```
...
supports `&&`-qualified call operators_), but can be used only with `WillOnce`.
...
operators_ to make that possible. See `gmock-actions.h` for examples.
